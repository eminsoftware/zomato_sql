...
-- find dublicates
select f_id, r_id, cuisine, price, count(*)
from menu
group by 1, 2, 3, 4
having count(*) > 1;
...
order by food_name, f_id;


-- simple functions

CREATE OR REPLACE FUNCTION get_restaurant_name(p_rid TEXT)
RETURNS TEXT 
AS $$
DECLARE
  r_name TEXT;
...
SELECT get_restaurant_name('100010') as restaurant_name;


create or replace function fn_user_name(p_user_id int)
returns varchar(40)
as $$

declare u_name varchar(40);
begin
...
select fn_user_name('44') as user_name;


-- materialized view

create materialized view mv_user_info as
select name, age
from users;
